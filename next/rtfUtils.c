...
char* parse_line ( struct FontInfoList* fi, struct FontInfo* (*changeList)[], char** rtfText, char** appendRTF );
void replace_rtfFlags ( unsigned char** rtfText, enum Fonts Fout, struct FontInfoList* fi );

void strip_rtf ( char** rtfText );
boolean rtf2ansi ( char* escape );
char* get_fontNumber ( char* headerBottom );
void reset_fontId ( struct FontInfoList* fi, struct FontInfo* changeList[], char** rtfLine );
...
char *tempString = returnString;
 ...
 /*****************************************************************************/
/*****************************************************************************/
/*  parse_line ( fi, changeList, rtfText, isGez, appendRTF )
/*
...
char*
parse_line ( fi, changeList, rtfText, appendRTF )
  struct FontInfoList* fi;
  struct FontInfo* (*changeList)[];
  char** rtfText;
  char** appendRTF;
...
boolean done=false;
  ...
  if ( fi->top->setId == Froman )
    {
  fprintf ( stderr, "  fi->top == Froman\n" );
  ...
  fprintf ( stderr, "           Top = %s\n", Top );
              reset_fontId ( fi, *changeList, &Top );
              if ( fi->top->setId != Froman )
                {
...
void 
replace_rtfFlags ( rtfText, Fout, fi )
 ...
 */
  ...
  if (fi->top->rtfId == NULL)
    same = false;
  else
    same = ( !strcmp(fi->font[nextFout]->rtfId, fi->top->rtfId) ) ? true : false;
 ...
 /*****************************************************************************/
/*****************************************************************************/
/*  reset_fontId ( changeList, rtfLine, topFont )
/*
...
void
reset_fontId ( fi, changeList, rtfLine )
  struct FontInfoList* fi;
  struct FontInfo* changeList[];
  char** rtfLine;
{
