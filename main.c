...
/*  Rip off RTF header, set up conversion routines, and let gezOut(s) do it's thing  */
  ...
  switch ( lethFlags->docIn )
    {
      case rtf:
        gezOut ( Fout, fi, &changeList, header, inFile, outFile, &isGez, fidelStats, statsFlags, lethFlags, HEADER );
        while ( !feof(inFile) )
          {
            if ( (rtfLine = get_rtfLine (inFile)) == NULL )
              break;
            gezOut (Fout, fi, &changeList, rtfLine, NULL, outFile, &isGez, fidelStats, statsFlags, lethFlags, RTFLINE);
            free ( (char *)rtfLine );
          }
        break;
      ...
      default    :
        gezOut ( Fout, fi, &changeList, header, inFile, outFile, &isGez, fidelStats, statsFlags, lethFlags, HEADER );
        gezOut ( Fout, fi, &changeList,   NULL, inFile, outFile, &isGez, fidelStats, statsFlags, lethFlags, PLAINLINE );
        break;
